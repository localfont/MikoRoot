...
define $(2)_CONFIGURE_CMDS
	rm -rf $$($$(PKG)_SRCDIR)/mikoos-build
	mkdir -p $$($$(PKG)_SRCDIR)/mikoos-build
	sed -e "/^\[binaries\]$$$$/s:$$$$:$$(foreach x,$$($(2)_MESON_EXTRA_BINARIES),\n$$(x)):" \
	    ...
	    > $$($$(PKG)_SRCDIR)/mikoos-build/cross-compilation.conf
	PATH=$$(BR_PATH) \
	CC_FOR_BUILD="$$(HOSTCC)" \
	...
	$$(MESON) setup \
		...
		--cross-file=$$($$(PKG)_SRCDIR)/mikoos-build/cross-compilation.conf \
		-Db_pie=false \
		...
		$$($$(PKG)_CONF_OPTS) \
		$$($$(PKG)_SRCDIR) $$($$(PKG)_SRCDIR)/mikoos-build
endef
else
...
define $(2)_BUILD_CMDS
	$$(TARGET_MAKE_ENV) $$($$(PKG)_NINJA_ENV) \
		$$(NINJA) $$(NINJA_OPTS) $$($$(PKG)_NINJA_OPTS) -C $$($$(PKG)_SRCDIR)/mikoos-build
endef
else
define $(2)_BUILD_CMDS
...
define $(2)_INSTALL_CMDS
	$$(HOST_MAKE_ENV) $$($$(PKG)_NINJA_ENV) \
		$$(NINJA) $$(NINJA_OPTS) -C $$($$(PKG)_SRCDIR)/mikoos-build install
endef
endif
