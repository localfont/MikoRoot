...
with MikoOS. You need not add this directory in your PATH to use
...
brmake
    ...
    ("'br.log' in the current directory), and just outputs the MikoOS
...
check-package
    a script that checks the coding style across the mikoos tree. It
...
docker-run
    ...
    mikoos CI docker container; pass no command to get an interactive
    shell.

genrandconfig
    a script that generates a random configuration, used by the autobuilders
    (http://autobuild.mikoos.org). It selects a random toolchain from
...
get-developers
    ...
    of MikoOS, so they can be Cc:ed on a mail. Accepts a patch as
    input, a package name or and architecture name.

scancpan
    a script to create a MikoOS package by scanning a CPAN module
    description.

scanpypi
    a script to create a MikoOS package by scanning a PyPI package
    description.

size-stats-compare
    a script to compare the rootfs size between two different MikoOS
