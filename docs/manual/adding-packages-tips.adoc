...
In MikoOS, there is some relationship between:
...
MikoOS provides a script in +utils/check-package+ that checks new or
...
Python PyPi packages +flake8+ and +python-magic+. The MikoOS code
...
MikoOS runs https://autobuild.mikoos.org/[autobuilders] which
...
MikoOS provides a script in +utils/test-pkg+ that uses the same base
...
MikoOS developers as being the most useful and representative
...
- The tarball name generated by github matches the default one from
  MikoOS (e.g.: +foo-f6fb6654af62045239caed5950bc6c7971965e60.tar.gz+),
...
xref:github-download-url[], MikoOS also provides the +gitlab+ macro
...
To make sure the token is used in the MikoOS build, add it to +~/.netrc+
