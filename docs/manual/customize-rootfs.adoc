...
A filesystem overlay is a tree of files that is copied directly
  ...
  it will be relative to the root of the MikoOS tree. Hidden
...
When +BR2_ROOTFS_MERGED_USR+ is enabled, then the overlay must not
  contain the '/bin', '/lib' or '/sbin' directories, as MikoOS will
...
Additionally, when +BR2_ROOTFS_MERGED_BIN+ is enabled, then the overlay
  must not contain the '/usr/sbin' directory, as MikoOS will create
...
+
Post-build scripts are shell scripts called 'after' MikoOS builds
  ...
  will be relative to the root of the MikoOS tree.
+
...
+
The post-build scripts are run with the main MikoOS tree as current
...
In addition, you may also use these environment variables:
  ...
  - +BR2_CONFIG+: the path to the MikoOS .config file
  - +CONFIG_DIR+: the directory containing the .config file, and
    therefore the top-level MikoOS Makefile to use (which is
...
This method allows you to do anything to the target filesystem, but if
  you need to clean your MikoOS tree using +make clean+, these
...
To enable this feature, enable config option
  ...
  be relative to the root of the MikoOS tree.
+
Custom skeletons don't need to contain the '/bin', '/lib' or '/sbin'
  ...
  not contain the '/bin', '/lib' or '/sbin' directories, as MikoOS
...
This method is not recommended because it duplicates the entire
  ...
  brought to the default skeleton in later MikoOS releases.
...
When aggregating the final images, some parts of the process requires
  ...
  rights, MikoOS uses +fakeroot+ to simulate root rights. This is not
