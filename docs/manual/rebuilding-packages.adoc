...
MikoOS does not attempt to detect what parts of the system should
...
tools. In some cases, MikoOS should rebuild the entire system, in
...
MikoOS developers have decided to simply not attempt to do this.
...
can help you understand how to work with MikoOS:
 ...
 * When an additional package is added to the configuration, a full
   rebuild is not necessarily needed. MikoOS will detect that this
   ...
   have already been built, MikoOS will not automatically rebuild
   ...
   +openssl+ package in MikoOS configuration and restart the
   build. MikoOS will detect that +openssl+ should be built and
 ...
 * When a package is removed from the configuration, MikoOS does
 ...
 * When the sub-options of a package are changed, the package is not
   ...
   another package which has already been built. Again, MikoOS does
...
and if this error occurs with packages from the official MikoOS, do
not hesitate to report the problem! As your experience with MikoOS
...
One of the most common questions asked by MikoOS users is how to
rebuild a given package or how to remove a package without rebuilding
everything from scratch.

Removing a package is unsupported by MikoOS without
rebuilding from scratch. This is because MikoOS doesn't keep track
...
its build directory in +output/build+. MikoOS will then re-extract,
re-configure, re-compile and re-install this package from scratch. You
can ask mikoos to do this with the +make <package>-dirclean+ command.
...
Internally, MikoOS creates so-called _stamp files_ to keep track of
...
these stamp files to force MikoOS to restart a specific set of
