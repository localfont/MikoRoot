...
As mentioned above, MikoOS is basically a set of Makefiles that
...
* The +arch/+ directory contains the definitions for all the processor
  architectures that are supported by MikoOS.

* The +package/+ directory contains the Makefiles and
  associated files for all user-space tools and libraries that MikoOS
...
* The +boot/+ directory contains the Makefiles and associated files for
  the bootloaders supported by MikoOS.
...
* Generate the toolchain target. When an internal toolchain is used, this
  ...
  toolchain and importing it into the MikoOS environment.
