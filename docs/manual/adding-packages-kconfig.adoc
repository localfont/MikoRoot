...
kernel, Busybox, and MikoOS itself. The presence of a .config file
and a +menuconfig+ target are two well-known symptoms of kconfig being
used.

MikoOS features an infrastructure for packages that use kconfig for
...
MikoOS, and to handle the copying back and forth of the configuration
...
In order to use the +kconfig-package+ infrastructure for a MikoOS
...
* +foo-diff-config+, which outputs the differences between the current
  configuration and the one defined in the MikoOS configuration for
...
* +FOO_KCONFIG_FIXUP_CMDS+: a list of shell commands needed to fixup the
  ...
  configuration of MikoOS, for example. By default, empty.
