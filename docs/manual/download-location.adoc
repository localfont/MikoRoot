...
The various tarballs that are downloaded by MikoOS are all stored
in +BR2_DL_DIR+, which by default is the +dl+ directory. If you want
to keep a complete version of MikoOS which is known to be working
...
If you maintain several MikoOS trees, it might be better to have a
shared download location. This can be achieved by pointing the
+BR2_DL_DIR+ environment variable to a directory. If this is
set, then the value of +BR2_DL_DIR+ in the MikoOS configuration is
