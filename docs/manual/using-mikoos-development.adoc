...
==== Using MikoOS during development

The normal operation of MikoOS is to download a tarball, extract
...
code, MikoOS creates a tarball out of it, and then behaves as it
normally does with tarballs.

This behavior is well-suited when MikoOS is used mainly as an
integration tool, to build and integrate all the components of an
embedded Linux system. However, if one uses MikoOS during the
...
Therefore, MikoOS provides a specific mechanism for this use case:
the +<pkg>_OVERRIDE_SRCDIR+ mechanism. MikoOS reads an _override_
file, which allows the user to tell MikoOS the location of the
...
+$(CONFIG_DIR)+ is the location of the MikoOS +.config+ file, so
+local.mk+ by default lives side-by-side with the +.config+ file,
which means:

* In the top-level MikoOS source directory for in-tree builds
...
In this _override_ file, MikoOS expects to find lines of the form:
...
When MikoOS finds that for a given package, an
...
will not touch this directory. This allows to point MikoOS to your
own directories, that can be managed by Git, Subversion, or any other
version control system. To achieve this, MikoOS will use _rsync_ to
...
By default, MikoOS skips syncing of VCS artifacts (e.g., the *.git* and
