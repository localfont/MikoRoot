...
== Contributing to MikoOS

There are many ways in which you can contribute to MikoOS: analyzing
...
improvements to MikoOS or its manual. The following sections give a
little more detail on each of these items.

If you are interested in contributing to MikoOS, the first thing you
should do is to subscribe to the MikoOS mailing list. This list is
the main way of interacting with other MikoOS developers and to send
...
git repository of MikoOS, rather than starting from an extracted
source code tarball. Git is the easiest way to develop from and directly
send your patches to the mailing list. Refer to xref:getting-mikoos[]
for more information on obtaining a MikoOS git tree.

=== Reproducing, analyzing and fixing bugs

A first way of contributing is to have a look at the open bug reports in
the https://gitlab.com/mikoos.org/mikoos/-/issues[MikoOS bug
...
The MikoOS autobuilders are a set of build machines that continuously
run MikoOS builds based on random configurations. This is done for
all architectures supported by MikoOS, with various toolchains, and
with a random selection of packages. With the large commit activity on
MikoOS, these autobuilders are a great help in detecting problems
very early after commit.

All build results are available at https://autobuild.mikoos.org[],
statistics are at https://autobuild.mikoos.org/stats.php[]. Every day,
...
- Fixing a problem. When fixing autobuild failures, you should follow
  ...
  . Check if you can reproduce the problem by building with the same
    ...
    https://gitlab.com/mikoos.org/mikoos-test/-/blob/master/utils/br-reproduce-build[br-reproduce-build]
    script that will automatically clone a MikoOS git repository,
  ...
  . Verify that the problem is really fixed by starting from a clean
    MikoOS tree and only applying your fix.
  . Send the fix to the MikoOS mailing list (see
    ...
    problem will be fixed in a later release, and the patch in MikoOS
    can be removed.
    In the commit message of a patch fixing an autobuild failure, add a
    reference to the build result directory, as follows:
----
Fixes: https://autobuild.mikoos.org/results/51000a9d4656afe9e0ea6f07b9f8ed374c2e4069
----
...
tags, without exception, and we encourage everyone to do this. MikoOS
...
MikoOS's Patchwork website can be used to pull in patches for testing
purposes. Please see xref:apply-patches-patchwork[] for more
information on using MikoOS's Patchwork website to apply patches.

[[apply-patches-patchwork]]
==== Applying Patches from Patchwork

The main use of MikoOS's Patchwork website for a developer is for
...
If you want to contribute to MikoOS but don't know where to start,
and you don't like any of the above topics, you can always work on items
from the http://elinux.org/MikoOS#Todo_list[MikoOS TODO list].
...
If you made some changes to MikoOS and you would like to contribute
them to the MikoOS project, proceed as follows.
...
under the MikoOS license (i.e. GPL-2.0+, except for package patches,
...
MikoOS provides a handy tool to check for common coding style
...
MikoOS provides a handy tool to know to whom your patches should be
...
----
$ git send-email --to mikoos@mikoos.org --cc bob --cc alice outgoing/*
----
...
----
$ git send-email --to mikoos@mikoos.org \
...
----
$ git config sendemail.to mikoos@mikoos.org
...
https://patchwork.ozlabs.org/project/mikoos/list/[patchwork], +git+
...
superseded in
https://patchwork.ozlabs.org/project/mikoos/list/[patchwork]. You
need to create an account on
https://patchwork.ozlabs.org/project/mikoos/list/[patchwork] to be
...
https://patchwork.ozlabs.org/project/mikoos/list/[patchwork] should
...
https://patchwork.ozlabs.org/project/mikoos/list/[patchwork]. The
...
+defconfig+; see xref:customize-store-mikoos-config[]).
...
MikoOS includes a run-time testing framework built upon Python
scripting and QEMU runtime execution. The goals of the framework are
the following:

* build a well defined MikoOS configuration
...
test_run (tests.toolchain.test_external.TestExternalToolchainMikoOSMusl) ... ok
test_run (tests.toolchain.test_external.TestExternalToolchainMikoOSuClibc) ... ok
...
Within the MikoOS repository, the testing framework is organized at the
...
* Defining the +config+ member of the test class, to the MikoOS
...
+TestInitSystemBusyboxRw/+ is the MikoOS output directory, and it
is preserved only if the +-k+ option is passed.

+TestInitSystemBusyboxRw-build.log+ is the log of the MikoOS build.
...
MikoOS make targets (in order to regenerate the complete image with
the new modifications) and then rerun the test.

==== Runtime tests and Gitlab CI

All runtime tests are regularly executed by MikoOS Gitlab CI
infrastructure, see .gitlab.yml and
https://gitlab.com/mikoos.org/mikoos/-/jobs.

You can also use Gitlab CI to test your new test cases, or verify that
existing tests continue to work after making changes in MikoOS.

In order to achieve this, you need to create a fork of the MikoOS
project on Gitlab, and be able to push branches to your MikoOS fork
on Gitlab.
