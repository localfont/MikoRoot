...
+ccache+ support is integrated in MikoOS. You just have to enable
...
+$HOME/.mikoos-ccache+. This default location is outside of
MikoOS output directory so that it can be shared by separate
MikoOS builds. If you want to get rid of the cache, simply remove
this directory.
...
To avoid this issue, mikoos has the +Use relative paths+ option
...
When +ccache+ is enabled in MikoOS using the +BR2_CCACHE=y+ option:

* +ccache+ is used during the MikoOS build itself

* +ccache+ is not used when building outside of MikoOS, for example
...
the MikoOS build), when unset or set to a value different from +1+,
