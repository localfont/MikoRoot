...
[[outside-br-custom]]
=== Keeping customizations outside of MikoOS
 ...
 * directly within the MikoOS tree, typically maintaining them using
   branches in a version control system so that upgrading to a newer
   MikoOS release is easy.

 * outside of the MikoOS tree, using the _br2-external_ mechanism.
   This mechanism allows to keep package recipes, board support and
   configuration files outside of the MikoOS tree, while still
...
One can tell MikoOS to use one or more br2-external trees by setting
the +BR2_EXTERNAL+ make variable set to the path(s) of the br2-external
tree(s) to use. It can be passed to any MikoOS +make+ invocation. It
...
interpreted relative to the main MikoOS source directory, *not* to
the MikoOS output directory.

.Note:
If using an br2-external tree from before MikoOS 2016.11, you need to
convert it before you can use it with MikoOS 2016.11 onward. See
xref:br2-external-converting[] for help on doing so.

Some examples:

----
mikoos/ $ make BR2_EXTERNAL=/path/to/foo menuconfig
----
...
----
mikoos/ $ make
mikoos/ $ make legal-info
----

We can switch to another br2-external tree at any time:

----
mikoos/ $ make BR2_EXTERNAL=/where/we/have/bar xconfig
----
...
----
mikoos/ $ make BR2_EXTERNAL="/path/to/foo /where/we/have/bar" menuconfig
----

Or disable the usage of any br2-external tree:

----
mikoos/ $ make BR2_EXTERNAL= xconfig
----
 ...
 * +name+, mandatory, defines the name for that br2-external tree. That
   ...
   other character is forbidden. MikoOS sets the variable
...
Since it is possible to use multiple br2-external trees at once, this
  name is used by MikoOS to generate variables for each of those trees.
...
in MikoOS itself) or other custom configuration options or make logic.

MikoOS automatically includes the +Config.in+ from each br2-external
...
MikoOS package recipes, as explained in xref:adding-packages[].
...
One can store MikoOS defconfigs in the +configs+ subdirectory of
the br2-external tree. MikoOS will automatically show them in the
...
to override a defconfig bundled in MikoOS or another br2-external
tree.

===== The +provides/+ directory

For some packages, MikoOS provides a choice between two (or more)
...
configuration file for which MikoOS allows to set the location (by
