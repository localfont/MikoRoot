...
The main MikoOS directory contains a file named +DEVELOPERS+ that
lists the developers involved with various areas of MikoOS. Thanks
...
- Find which developers are taking care of a given architecture or
  ...
  MikoOS's autobuild infrastructure.

We ask developers adding new packages, new boards, or generally new
functionality in MikoOS, to register themselves in the +DEVELOPERS+
...
- When using the +-c+ command line option, +get-developers+ will look
  at all files under version control in the MikoOS repository, and
