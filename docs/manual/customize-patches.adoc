...
those provided in MikoOS. This might be used to support custom
...
specify a patch directory for any package in mikoos. It should also
...
MikoOS bundles a xref:adding-packages-hash[list of hashes] against
...
or a VCS repository location and changeset), MikoOS can't carry
hashes for those.

For users concerned with the integrity of such downloads, it is possible
to provide a list of hashes that MikoOS can use to check arbitrary
