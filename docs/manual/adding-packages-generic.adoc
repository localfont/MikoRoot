...
MikoOS will ensure that all these packages are built and installed
...
+
  1. due to local caching, MikoOS will not re-fetch the repository,
  ...
  2. because two builds can never be perfectly simultaneous, and because
     ...
     two users, using the same MikoOS tree and building the same
...
* +LIBFOO_SOURCE+ may contain the name of the tarball of the package,
  which MikoOS will use to download the tarball from
...
* +LIBFOO_PATCH+ may contain a space-separated list of patch file
  names, that MikoOS will download and apply to the package source
  code. If an entry contains +://+, then MikoOS will assume it is a
  full URL and download the patch from this location. Otherwise,
  MikoOS will assume that the patch should be downloaded from
  +LIBFOO_SITE+. If +HOST_LIBFOO_PATCH+ is not specified, it defaults
  to +LIBFOO_PATCH+. Note that patches that are included in MikoOS
  itself use a different mechanism: all files of the form
  +*.patch+ present in the package directory inside
  MikoOS will be applied to the package after extraction (see
  ...
  in the MikoOS package directory.
...
* +LIBFOO_SITE+ provides the location of the package, which can be a
  ...
  trailing slash: it will be added by MikoOS between the directory
...
* +LIBFOO_EXTRA_DOWNLOADS+ is a space-separated list of additional
  files that MikoOS should download. If an entry contains +://+
  then MikoOS will assume it is a complete URL and will download
  the file using this URL. Otherwise, MikoOS will assume the file
  to be downloaded is located at +LIBFOO_SITE+. MikoOS will not do
...
* +LIBFOO_SITE_METHOD+ determines the method used to fetch or copy the
  package source code. In many cases, MikoOS guesses the method
  ...
  ** +svn+ for retrieving source code from a Subversion repository.
     ...
     MikoOS performs a checkout which is preserved as a tarball in
  ...
  ** +local+ for a local source code directory. One should use this
     ...
     the package source code. MikoOS copies the contents of the
...
* +LIBFOO_STRIP_COMPONENTS+ is the number of leading components
  ...
  "<pkg-name>-<pkg-version>", thus MikoOS passes
...
* +LIBFOO_DEVICES+ lists the device files to be created by MikoOS
...
* +LIBFOO_ACTUAL_SOURCE_TARBALL+ only applies to packages whose
  ...
  name of the actual source code archive and MikoOS will download
...
* +LIBFOO_REDISTRIBUTE+ can be set to +YES+ (default) or +NO+ to indicate if
  ...
  non-opensource packages: MikoOS will not save the source code for this
...
* +LIBFOO_IGNORE_CVES+ is a space-separated list of CVEs that tells
  MikoOS CVE tracking tools which CVEs should be ignored for this
  ...
  some reason does not affect the MikoOS package. A Makefile comment must
  always precede the addition of a CVE to this variable. Example:
+
----
# 0001-fix-cve-2020-12345.patch
LIBFOO_IGNORE_CVES += CVE-2020-12345
# only when built with libbaz, which MikoOS doesn't support
...
* +LIBFOO_EXTRACT_CMDS+ lists the actions to be performed to extract
  ...
  automatically handled by MikoOS. However, if the package uses a
...
* +LIBFOO_INSTALL_INIT_SYSV+, +LIBFOO_INSTALL_INIT_OPENRC+ and
  ...
  When systemd is used as the init system, mikoos will automatically enable
  ...
  being automatically enabled by mikoos.
...
* +$(LIBFOO_PKGDIR)+ contains the path to the directory containing the
  ...
  necessary to install a file bundled in MikoOS, like a runtime
...
* +$(LIBFOO_DL_DIR)+ contains the path to the directory where all the downloads
  made by MikoOS for +libfoo+ are stored in.
