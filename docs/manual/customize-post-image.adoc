...
relative to the root of the MikoOS tree.

Just like post-build scripts, post-image scripts are run with the main
MikoOS tree as current working directory. The path to the +images+
...
MikoOS, which should normally _not_ be the root user. Therefore, any
