...
MikoOS allows to do cross-debugging, where the debugger runs on the
build machine and communicates with +gdbserver+ on the target to
control the execution of the program.

To achieve this:

* If you are using an _internal toolchain_ (built by MikoOS), you
...
* If you are using an _external toolchain_, you should enable
  ...
  possible to let MikoOS build them, by enabling the same options as
...
----
<mikoos>/output/host/bin/<tuple>-gdb -ix <mikoos>/output/staging/usr/share/mikoos/gdbinit foo
----
...
The +<mikoos>/output/staging/usr/share/mikoos/gdbinit+ file will tell the
