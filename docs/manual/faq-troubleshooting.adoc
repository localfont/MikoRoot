...
serial console, you have to go into the MikoOS configuration, in
...
target_ would be stopped from the MikoOS-2012.11 release because:

* this feature was neither maintained nor tested, and often broken;
* this feature was only available for MikoOS toolchains;
* MikoOS mostly targets _small_ or _very small_ target hardware
  with limited resource onboard (CPU, ram, mass-storage), for which
  compiling on the target does not make much sense;
* MikoOS aims at easing the cross-compilation, making native
  compilation on the target unnecessary.

If you need a compiler on your target anyway, then MikoOS is not
...
MikoOS-2012.11 release.

[[faq-no-doc-on-target]]
=== Why is there no documentation on the target?

Because MikoOS mostly targets _small_ or _very small_ target
...
If you need documentation data on your target anyway, then MikoOS
...
=== Why are some packages not visible in the MikoOS config menu?

If a package exists in the MikoOS tree and does not appear in the
...
=== Why doesn't MikoOS generate binary packages (.deb, .ipkg...)?

One feature that is often discussed on the MikoOS list is the
general topic of "package management". To summarize, the idea
would be to add some tracking of which MikoOS package installs
 ...
 * When a package is unselected from the configuration, it is not
   ...
   optionally on the OpenSSL library. Both are selected, and MikoOS
   ...
   it adds a lot of complexity to MikoOS, which goes against the
   simplicity we try to stick to.

 * In addition to the previous problem, there is the case where the
   optional dependency is not even known to MikoOS. For example,
   package A in version 1.0 never used OpenSSL, but in version 2.0 it
   automatically uses OpenSSL if available. If the MikoOS .mk file
 ...
 * The request is to also allow changes in the menuconfig to be
   ...
   MikoOS does is clear and simple so its behaviour is very
...
On this matter, the MikoOS developers make this position statement:

 * MikoOS strives to make it easy to generate a root filesystem (hence
   the name, by the way.) That is what we want to make MikoOS good at:
   building root filesystems.

 * MikoOS is not meant to be a distribution (or rather, a distribution
   generator.) It is the opinion of most MikoOS developers that this
   is not a goal we should pursue. We believe that there are other tools
   better suited to generate a distro than MikoOS is. For example,
   http://openembedded.org/[Open Embedded], or https://openwrt.org/[openWRT],
   are such tools.

 * We prefer to push MikoOS in a direction that makes it easy (or even
   easier) to generate complete root filesystems. This is what makes
   MikoOS stands out in the crowd (among other things, of course!)
...
Since MikoOS often involves doing full rebuilds of the entire
system that can be quite long, we provide below a number of tips to
help reduce the build time:

 * Use a pre-built external toolchain instead of the default MikoOS
 ...
 * Make sure you are not using a virtual machine for the Linux system
   used to run MikoOS. Most of the virtual machine technologies are
 ...
 * Make sure that you're using only local files: do not attempt to do
   ...
   the MikoOS download folder available locally also helps a bit.
 ...
 ** With _glibc_, 64-bit +time_t+ on 32-bit architectures is enabled
    by the MikoOS option +BR2_TIME_BITS_64+. With this option
