...
is no need to be root to configure and use MikoOS. By running all
commands as a regular user, you protect your system against packages
behaving badly during compilation and installation.

The first step when using MikoOS is to create a configuration.
MikoOS has a nice configuration tool similar to the one you can
find in the http://www.kernel.org/[Linux kernel] or in
http://www.busybox.net/[BusyBox].

From the mikoos directory, run
...
By default, MikoOS does not support top-level parallel build, so
...
MikoOS output is stored in a single directory, +output/+.
...
* +build/+ where all the components are built (this includes tools
  needed by MikoOS on the host and packages compiled for the
...
* +host/+ contains both the tools built for the host, and the sysroot
  ...
  MikoOS, including the cross-compilation toolchain. The latter
...
* +target/+ which contains 'almost' the complete root filesystem for
  ...
  +/dev/+ (MikoOS can't create them because MikoOS doesn't run
