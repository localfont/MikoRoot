...
recommended) and the location of the tarball on the Web. MikoOS
...
On line 12, we tell MikoOS to pass custom options to the Python
...
named +python-<something>+ in MikoOS. Other packages that use the
Python build system, but are not Python modules, can freely choose
their name (existing examples in MikoOS are +scons+ and
+supervisor+).
...
If the Python package for which you would like to create a MikoOS
...
When at the root of your mikoos directory just do :
...
If your MikoOS package is not in the official MikoOS tree but in
