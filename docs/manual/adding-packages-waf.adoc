...
On line 10, we tell MikoOS what options to enable for libfoo.

On line 11, we tell MikoOS the dependencies of libfoo.
...
* +LIBFOO_NEEDS_EXTERNAL_WAF+ can be set to +YES+ or +NO+ to tell
  MikoOS to use the bundled +waf+ executable. If set to +NO+, the
  default, then MikoOS will use the waf executable provided in the
  package source tree; if set to +YES+, then MikoOS will download,
