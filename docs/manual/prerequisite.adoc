...
MikoOS is designed to run on Linux systems.

While MikoOS itself will build most host packages it needs for the
...
+
Some features or utilities in MikoOS, like the legal-info, or the
...
available through a version control system. Moreover, MikoOS is
