...
directory and submitted to the MikoOS upstream project. How to add
packages to MikoOS in general is explained in full detail in
...
However, MikoOS will not be aware of the packages in this location,
unless we perform some additional steps. As explained in
xref:adding-packages[], a package in MikoOS basically consists of two
files: a +.mk+ file (describing how to build the package) and a
+Config.in+ file (describing the configuration options for this
package).

MikoOS will automatically include the +.mk+ files in first-level
subdirectories of the +package+ directory (using the pattern
+package/\*/*.mk+). If we want MikoOS to include +.mk+ files from
...
merges with future MikoOS versions easier.
