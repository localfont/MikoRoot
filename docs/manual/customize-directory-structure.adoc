...
When customizing MikoOS for your project, you will be creating one or
...
specified in the MikoOS configuration, the MikoOS developers
...
this structure itself: either inside the MikoOS tree, or outside of
...
Note: if you choose to place this structure outside of the MikoOS
...
Almost all of the customization methods available in MikoOS, like
...
You normally launch MikoOS from the mikoos source directory, pointing
...
these variables and calls into mikoos.
...
A basic Makefile looks like this. It assumes the mikoos source is available
(e.g. as a git submodule) in the +mikoos+ subdirectory. It makes sure that
...
# Put downloads in this directory instead of in the MikoOS directory
ifeq ($(BR2_DL_DIR),)
...
MAKE_BUILDROOT = $(MAKE) -C $(THIS_EXTERNAL_PATH)/mikoos BR2_EXTERNAL=$(THIS_EXTERNAL_PATH)
