...
and the location of the tarball on the Web. MikoOS will automatically
download the tarball from this location.

On line 10, we tell MikoOS to install the package to the staging
...
On line 11, we tell MikoOS to not install the package to the
...
On line 12, we tell MikoOS to pass custom options to CMake when it is
