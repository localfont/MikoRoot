...
MikoOS offers an infrastructure to automatically handle this during
the builds. It supports three ways of applying patch sets: downloaded patches,
patches supplied within mikoos and patches located in a user-defined
...
then MikoOS will assume it is a full URL and download the patch
from this location. Otherwise, MikoOS will assume that the patch should be
...
Most patches are provided within MikoOS, in the package
...
.Notes
- The patch files coming with MikoOS should not contain any package version
...
. If there are some +*.patch+ files in the package's MikoOS
...
If a new issue is identified in MikoOS and upstream is generally affected by 
the issue (it's not a MikoOS specific issue), users should submit the patch
...
----
Upstream: N/A <additional information about why patch is MikoOS specific>
----
