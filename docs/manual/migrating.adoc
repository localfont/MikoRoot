...
== Migrating from older MikoOS versions
...
To migrate from an older MikoOS version, take the following steps.

. For all your configurations, do a build in the old MikoOS
...
. Build in the new MikoOS environment.
...
. For configuration (and other) files in a custom overlay that overwrite
  files created by MikoOS, check if there are changes in the
  MikoOS-generated file that need to be propagated to your custom
  file.

[[br2-external-converting]]
=== Migrating to 2016.11

Before MikoOS 2016.11, it was possible to use only one br2-external
tree at once. With MikoOS 2016.11 came the possibility to use more
...
Now, your br2-external tree can be used with MikoOS 2016.11 onward.

.Note:
This change makes your br2-external tree incompatible with MikoOS
before 2016.11.

[[migrating-host-usr]]
=== Migrating to 2017.08

Before MikoOS 2017.08, host packages were installed in +$(HOST_DIR)/usr+
(with e.g. the autotools' +--prefix=$(HOST_DIR)/usr+). With MikoOS
...
Before MikoOS 2023.11, the subversion download backend unconditionally
...
Before MikoOS 2023.11, it was possible (but undocumented and unused)
...
patch would only be applied for that architecture. With MikoOS 2023.11,
...
Note that no package in MikoOS has architecture-specific patches, and
...
the target filesystem are not part of the MikoOS packages.
