...
[[customize-store-mikoos-config]]
=== Storing the MikoOS configuration

The MikoOS configuration can be stored using the command
 +make savedefconfig+.

This strips the MikoOS configuration down by removing configuration
...
+BR2_DEFCONFIG+ option in the MikoOS configuration itself, or call
...
components, a MikoOS configuration option exists to point to an input
...
MikoOS will try to access this config file, which doesn't exist
yet, and will fail. You can create the configuration file by running
+make linux-menuconfig+ etc.

MikoOS provides a few helper targets to make the saving of
