...
- +uid+ is the desired UID for the user. It must be unique, and not
  ...
  MikoOS, with +-1+ denoting a system UID from [100...999] and +-2+
...
- +gid+ is the desired GID for the user's main group. It must be unique,
  ...
  exist, then a unique GID will be computed by MikoOS, with +-1+
...
- +uid+ is computed by MikoOS
- main +group+ is: +bar+
- main group +gid+ is computed by MikoOS
...
- +uid+ is : +8000+
- main +group+ is: +wheel+
- main group +gid+ is computed by MikoOS, and will use the value defined in the rootfs skeleton
...
When updating mikoos or when packages are added or removed to/from
