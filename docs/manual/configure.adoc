...
MikoOS has nothing to do with it (other than using it to build a
...
MikoOS provides two solutions for the cross-compilation toolchain:

 * The *internal toolchain backend*, called +MikoOS toolchain+ in
...
The _internal toolchain backend_ is the backend where MikoOS builds
 ...
 * Select a number of toolchain options (uClibc only): whether the
   ...
   MikoOS menus will change: many applications and libraries require
   ...
   all packages in MikoOS are tested against the default uClibc
   configuration bundled in MikoOS: if you deviate from this
...
cross-compilation toolchains. MikoOS knows about a number of
...
* Use a predefined external toolchain profile, and let MikoOS
  download, extract and install the toolchain. MikoOS already knows
...
* Use a predefined external toolchain profile, but instead of having
  MikoOS download and extract the toolchain, you can tell MikoOS
...
* Use a completely custom external toolchain. This is particularly
  useful for toolchains generated using crosstool-NG or with MikoOS
  ...
  to tell MikoOS what your external toolchain supports. If your
  ...
  library, then you have to tell MikoOS if it supports RPC,
  wide-char, locale, program invocation, threads and C++.
  At the beginning of the execution, MikoOS will tell you if
...
MikoOS itself. In general, all toolchains that support the
...
programs. Therefore, MikoOS cannot import the 'sysroot' of the
toolchain, as it would contain hundreds of megabytes of pre-compiled
libraries that are normally built by MikoOS.
...
C/C++ library), so we cannot import it properly into the MikoOS
...
MikoOS or crosstool-NG.

If you want to generate a custom toolchain for your project, that can
be used as an external toolchain in MikoOS, our recommendation is
to build it either with MikoOS itself (see
xref:build-toolchain-with-mikoos[]) or with
...
* If your pre-built external toolchain has a bug, may be hard to get a
  ...
  toolchain by yourself using MikoOS or Crosstool-NG.

[[build-toolchain-with-mikoos]]
==== Build an external toolchain with MikoOS

The MikoOS internal toolchain option can be used to create an
external toolchain. Here are a series of steps to build an internal
toolchain and package it up for reuse by MikoOS itself (or other
projects).

Create a new MikoOS configuration, with the following details:

* Select the appropriate *Target options* for your target CPU
  architecture

* In the *Toolchain* menu, keep the default of *MikoOS toolchain*
...
Then, we can trigger the build, and also ask MikoOS to generate a
...
+arm-mikoos-linux-uclibcgnueabi_sdk-mikoos.tar.gz+. Save this
tarball, as it is now the toolchain that you can re-use as an external
toolchain in other MikoOS projects.

In those other MikoOS projects, in the *Toolchain* menu:
...
When using an external toolchain, MikoOS generates a wrapper program,
...
Under +System configuration+, +/dev management+, MikoOS offers four
 ...
 * The first solution is *Static using device table*. This is the old
   ...
   devices are added or removed from the system. MikoOS therefore
   ...
   MikoOS source code. This file is processed when MikoOS
   ...
   default device table used by MikoOS, or to add an additional
   device table, so that additional _device files_ are created by
   MikoOS during the build. So, if you use this method, and a
 ...
 * The second solution is *Dynamic using devtmpfs only*. _devtmpfs_ is
   ...
   +CONFIG_DEVTMPFS+ and +CONFIG_DEVTMPFS_MOUNT+. When MikoOS is in
   ...
   build your Linux kernel outside of MikoOS, then it is your
   responsibility to enable those two options (if you fail to do so,
   your MikoOS system will not boot).
...
The MikoOS developers recommendation is to start with the *Dynamic
...
MikoOS allows to use three different types of init systems, which
 ...
 * The first solution is *BusyBox*. Amongst many programs, BusyBox has
   ...
   the default solution in MikoOS. The BusyBox +init+ program will
   ...
   +inittab+). The default +inittab+ in MikoOS is stored in
 ...
 * The second solution is *systemV*. This solution uses the old
   traditional _sysvinit_ program, packed in MikoOS in
...
The solution recommended by MikoOS developers is to use the
