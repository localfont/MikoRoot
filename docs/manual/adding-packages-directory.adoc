...
As a convention specific to MikoOS, the ordering of the attributes
is as follows:
...
dependencies are enabled. Typically, MikoOS uses the following
rules:
...
* Use a +depends on+ type of dependency when the user really needs to
  be aware of the dependency. Typically, MikoOS uses this type of
...
==== Dependencies on a Linux kernel built by mikoos

Some packages need a Linux kernel to be built by mikoos. These are
...
downloaded file, the hash(es) computed by MikoOS (after download) must
match the hash(es) stored in the +.hash+ file. If one or more hashes do
not match, MikoOS considers this an error, deletes the downloaded file,
and aborts.

If the +.hash+ file is present, but it does not contain a hash for a
downloaded file, MikoOS considers this an error and aborts. However,
...
mercurial) because MikoOS currently does not generate reproducible
...
repository location and changeset), MikoOS can't carry hashes for
...
at boot.  MikoOS comes with support for several init systems, some
...
that is preferred over a mikoos specific one, if it is compatible
with mikoos.
