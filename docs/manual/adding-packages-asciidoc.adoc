...
The MikoOS manual, which you are currently reading, is entirely written
...
Although MikoOS only contains one document written in AsciiDoc, there
...
a br2-external tree to match the MikoOS documentation, as it will be
...
MikoOS will not attempt to download anything to render a document.
...
* +FOO_POST_RSYNC_HOOKS+ to run additional commands after the sources
  have been copied by MikoOS. This can for example be used to
  generate part of the manual with information extracted from the
  tree. As an example, MikoOS uses this hook to generate the tables
  in the appendices.
...
MikoOS sets the following variable that can be used in the definitions
above:
