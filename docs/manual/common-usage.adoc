...
As default, everything built by MikoOS is stored in the directory
+output+ in the MikoOS tree.

MikoOS also supports building out of tree with a syntax similar to
...
----
 $ cd /tmp/build; make O=$PWD -C path/to/mikoos menuconfig
----

All the output files will be located under +/tmp/build+. If the +O+
path does not exist, MikoOS will create it.
...
relative to the main MikoOS source directory, *not* the current working
directory.

When using out-of-tree builds, the MikoOS +.config+ and temporary
files are also stored in the output directory. This means that you can
safely run multiple builds in parallel using the same source tree as
long as they use unique output directories.

For ease of use, MikoOS generates a Makefile wrapper in the output
...
MikoOS also honors some environment variables, when they are passed
...
* +UCLIBC_CONFIG_FILE=<path/to/.config>+, path to
  ...
  configuration interface, so through the MikoOS +.config+ file; this
...
* +BR2_CCACHE_DIR+ to override the directory where
  MikoOS stores the cached files when using ccache.
  +
* +BR2_DL_DIR+ to override the directory in which
  MikoOS stores/retrieves downloaded files.
  +
  Note that the MikoOS download directory can also be set from the
  configuration interface, so through the MikoOS +.config+ file. See
...
MikoOS can produce a JSON blurb that describes the set of enabled
...
MikoOS can also produce details about packages as HTML and JSON
...
Based on the output of +show-info+ MikoOS can generate a SBOM in
...
cat sbom.cdx.json | support/scripts/cve-check --nvd-path dl/mikoos-nvd/
----
...
One of MikoOS's jobs is to know the dependencies between packages,
...
embedded Linux system, MikoOS is capable of generating dependency
graphs.
...
such build time analysis, MikoOS collects the build time of each
...
how much each MikoOS package is contributing to the overall root
filesystem size. To help with such an analysis, MikoOS collects
...
To compare the root filesystem size of two different MikoOS compilations,
for example after adjusting the configuration or when switching to another
MikoOS release, use the +size-stats-compare+ script. It takes two
...
MikoOS has always been capable of using parallel build on a per
package basis: each package is built by MikoOS using +make -jN+ (or
...
Until 2020.02, MikoOS was however building packages in a serial
fashion: each package was built one after the other, without
parallelization of the build between packages. As of 2020.02,
MikoOS has experimental support for *top-level parallel build*,
...
. Enable the option +BR2_PER_PACKAGE_DIRECTORIES+ in the MikoOS
configuration

. Use +make -jN+ when starting the MikoOS build
