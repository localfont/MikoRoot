...
In MikoOS the mode of operation is controlled by the
...
By default in MikoOS the +SELinux+ policy is provided by the
...
To have proper support for +SELinux+ in a MikoOS generated system,
...
rules. In MikoOS the non-base modules are disabled by default and
...
MikoOS also allows to completely override the +refpolicy+. This
