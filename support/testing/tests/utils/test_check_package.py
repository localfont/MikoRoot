...
class TestCheckPackage(unittest.TestCase):
    ...
    def test_run(self):
        ...
        self.assert_file_was_processed(m)
        self.assert_warnings_generated_for_file(m)
        self.assertIn("{}:1: should be 80 hashes (https://nightly.mikoos.org/#writing-rules-mk)".format(rel_file), w)

        w, m = call_script(["check-package", "-b", abs_file],
                           self.WITH_UTILS_IN_PATH, infra.basepath())
        self.assert_file_was_processed(m)
        self.assert_warnings_generated_for_file(m)
        self.assertIn("{}:1: should be 80 hashes (https://nightly.mikoos.org/#writing-rules-mk)".format(abs_file), w)
