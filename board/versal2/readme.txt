This document describes the MikoOS support for the following
...
xilinx-prebuilt, you simply need to change the following MikoOS options:
...
by Xilinx Vivado, MikoOS needs access to the prebuilt image. The MikoOS
xilinx-prebuilt package has support for Versal2 XSA files exported from Xilinx
Vivado.

Using the option BR2_TARGET_XILINX_PREBUILT_VERSAL_XSA, MikoOS can obtain
the prebuilt PDI from the XSA file.

1) Start with a defconfig supported by MikoOS (e.g. VEK385)
    make versal2_vek385_defconfig
...
MikoOS generated SD card image without updating the OSPI boot.bin image, so
...
2) The default u-boot bootcmd is not the distro_bootcmd that mikoos
